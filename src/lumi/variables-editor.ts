...
function vePrimitiveText(value: unknown): string {
  if (value === null) return 'null';
...
function veCloneJson(value: unknown): JsonValue {
  return structuredClone(value) as JsonValue;
}

function vePathText(path: GuiPath): string {
  return path.join(' › ');
}
...
function veButton(label: string, className = ''): HTMLButtonElement {
...
function veModalBase(root: ShadowRoot, title: string): { overlay: HTMLElement; modal: HTMLElement; close(): void } {
  ...
  const modalLayer = root.querySelector<HTMLElement>('.status-body');
  (modalLayer ?? root).appendChild(overlay);
  return { overlay, modal, close };
}

function veModalActions(modal: HTMLElement, close: () => void, saveLabel: string, onSave: () => void): void {
...
function veShowAdd(
  ...
  veModalActions(dialog.modal, dialog.close, 'Add', () => {
    ...
    if (!key) return;
    let value: JsonValue;
    if (structure.value === 'text') value = '';
    else if (structure.value === 'number') value = 0;
    else if (structure.value === 'boolean') value = false;
...
export function renderVariablesEditor(root: ShadowRoot, options: VariablesEditorOptions): HTMLElement {
