...
export interface MessageEditedPayload { chatId: string; message: LumiChatMessage }
...
export interface SpindleApiLite {
  contracts?: { preAssemblyGenerationContext?: number };
  userStorage: UserStorageApi;
  chat: { getMessages(chatId: string): Promise<LumiChatMessage[]> };
  permissions: {
    has(permission: string): boolean;
    onChanged(handler: (detail: { permission: string; granted: boolean; allGranted: string[]; extensionId?: string }) => void): (() => void) | void;
    onDenied?(handler: (detail: { permission: string; operation: string }) => void): (() => void) | void;
  };
  log: { info(message: string, ...args: unknown[]): void; warn(message: string, ...args: unknown[]): void; error(message: string, ...args: unknown[]): void };
  registerContextHandler(handler: (context: ContextHandlerContext) => Promise<ContextHandlerContext>, priority?: number, options?: { timeoutMs?: number }): void;
  registerInterceptor(handler: (messages: LumiLlmMessage[], context: InterceptorContext) => Promise<LumiLlmMessage[] | { messages: LumiLlmMessage[]; breakdown?: Array<{ messageIndex: number; name?: string }> }>, priority?: number): void;
  ...
  on(event: 'MESSAGE_EDITED', handler: (payload: MessageEditedPayload, userId?: string) => void | Promise<void>): (() => void) | void;
