...
export function setup(ctx: SpindleFrontendContextLite) {
  ...
  const diagnosticsToggle = make('button', 'ffsm-toolbar-btn') as HTMLButtonElement;
  ...
  let diagnosticReport: Record<string, unknown> | null = null;
  ...
  function clearDiagnosticTrace(): void {
    diagnosticsBusy = true;
  ...
  function downloadPortableSnapshot(value: PortableSnapshot): void {
  ...
  function requestPortableSnapshot(action: 'copy' | 'download'): void {
    if (
      !activeChatId ||
      !snapshot?.ok ||
  ...
  function requestSnapshotRestoreText(text: string): void {
    if (
      !activeChatId ||
      busy ||
      !snapshot?.ok ||
    ...
    try {
      portable = JSON.parse(text) as PortableSnapshot;
    } catch (error) {
  ...
  function renderNewGame(): HTMLElement {
    ...
    portableToggle.addEventListener('click', () => {
      ...
      if (portableImportOpen) legacyImportOpen = false;
      render();
    });
    portableCard.appendChild(portableToggle);
    if (portableImportOpen) {
      const fileInput = make('input', 'ffsm-input') as HTMLInputElement;
      fileInput.type = 'file';
      ...
      const portableText = make('textarea', 'ffsm-textarea') as HTMLTextAreaElement;
      ...
      const importPortableButton = make('button', 'ffsm-btn', 'Import Portable Snapshot') as HTMLButtonElement;
      ...
      importPortableButton.addEventListener('click', () => {
        ...
        let portable: PortableSnapshot;
        try {
          portable = JSON.parse(portableText.value) as PortableSnapshot;
        } catch (error) {
    ...
    importToggle.addEventListener('click', () => {
      ...
      if (legacyImportOpen) portableImportOpen = false;
      render();
    });
  ...
  function renderDiagnosticOverlay(): HTMLElement {
    ...
    const refresh = make('button', 'ffsm-btn', diagnosticsBusy ? 'Collecting…' : 'Snapshot') as HTMLButtonElement;
    refresh.disabled = diagnosticsBusy;
    refresh.addEventListener('click', requestDiagnostics);

    const clear = make('button', 'ffsm-btn', 'Clear Trace') as HTMLButtonElement;
    ...
    const copy = make('button', 'ffsm-btn', 'Copy') as HTMLButtonElement;
    ...
    const text = make('textarea', 'ffsm-diagnostic-text') as HTMLTextAreaElement;
    text.readOnly = true;
    ...
    panel.appendChild(make(
      'div',
      'ffsm-diagnostic-note',
      diagnosticNotice || 'Read-only snapshot: lifecycle trace, transcript hashes, semantic/store heads, active variant/attempt lineage, projection binding, and Scene.HPH shape. No state values are edited.',
    ));
    ...
    if (diagnosticsOpen) app.appendChild(renderDiagnosticOverlay());
    applyPanelHeight();
    syncPanelVisibility();
  }
  ...
  const backendUnsub = ctx.onBackendMessage((payload: any) => {
    ...
    if (payload?.type === 'ffmvu_snapshot_export_result') {
      if (payload.chatId && payload.chatId !== activeChatId) return;
      if (snapshotExportRequestId && payload.requestId !== snapshotExportRequestId) return;
    ...
    if (payload?.type === 'ffmvu_snapshot_import_result') {
      ...
      if (payload.ok) {
        ...
        if (snapshot?.initialized) snapshotExportNotice = reason;
        else notice = reason;
        render();
        if (snapshot?.initialized) requestState();
      }
      return;
    }
