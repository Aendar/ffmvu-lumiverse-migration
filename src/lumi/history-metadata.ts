...
export function injectNarrativeHistoryContext(
  ...
  for (let i = 0; i < out.length; i++) {
    const message = out[i];
    if (message.role !== 'assistant' || typeof message.content !== 'string' || !message.sourceMessageId) continue;
    ...
    if (!message.content.startsWith('<narrative_time ')) message.content = marker + '\n' + message.content;
    tagged += 1;
  }
  ...
  if (systemIndex >= 0) out[systemIndex] = { ...out[systemIndex], content: String(out[systemIndex].content) + '\n\n' + rules };
  else out.unshift({ role: 'system', content: rules });
  return out;
}
