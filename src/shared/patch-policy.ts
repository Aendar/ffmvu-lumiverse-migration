...
export function buildModelPatchAuthorizationView(view: PromptView): ModelPatchAuthorizationView {
...
function deny(message: string): never { throw new Error('MODEL_PATCH_UNAUTHORIZED: ' + message); }

function authorizeProjectedCollection(
  ...
  if (!id) deny(label + ' id missing');
  const exists = hasOwn(baseCollection, id);
  if (exists) {
    if (!visibleIds.includes(id)) deny(label + ' entity omitted from frozen projection: ' + id);
    return;
  }
  if (operation.op !== 'add' || parts.length !== idIndex + 1) deny('new ' + label + ' must be one exact add at entity root: ' + id);
}
...
export function assertModelPatchAuthorization(
  ...
  if (authorization.version !== MODEL_PATCH_AUTHORIZATION_VERSION) deny('unsupported authorization version');
  ...
  for (const operation of operations) {
    ...
    if (!parts.length) deny('state root is not writable');
    if (parts.some(part => routingSegments.has(part))) deny('projection routing label is not writable: ' + operation.path);
    if (parts[0] === 'ProjectionMeta') deny('ProjectionMeta is read-only');

    if (parts[0] === 'World') {
      if (parts.length < 2) deny('World root replacement is not allowed');
      continue;
    }
    if (parts[0] === 'Mainchar') {
      if (parts.length < 2) deny('Mainchar root replacement is not allowed');
      continue;
    }
    if (parts[0] === 'World_Calc') {
      const section = parts[1] as keyof ModelPatchAuthorizationView['worldCalc'];
      if (!['Factions', 'Locations', 'Ruins', 'Events'].includes(section)) deny('unknown World_Calc section: ' + String(parts[1]));
    ...
    if (parts[1] === 'NextNpcId') {
      if (parts.length !== 2) deny('Narrative.NextNpcId must be written as one scalar');
      nextNpcOps.push(operation);
      continue;
    }
    if (parts[1] === 'Scene') {
      if (parts.length < 3) deny('Narrative.Scene root replacement is not allowed');
      continue;
    }
    if (parts[1] === 'NPCs') {
      const id = parts[2];
    ...
    if (parts[1] === 'GM_Notes') {
      if (parts[2] !== 'Active') deny('only GM_Notes.Active is model-writable');
    ...
    if (parts[1] === 'WorldSim') {
      if (parts[2] === 'LastShift' && parts.length === 3) continue;
      if (parts[2] === 'Threads') {
  ...
  if (uniqueNpcIds.length) {
    ...
    for (let i = 0; i < numeric.length; i++) {
      ...
      if (numeric[i].n !== expected) deny('new NPC id must allocate contiguously from NextNpcId; expected npc_' + String(expected).padStart(4, '0') + ', got ' + numeric[i].id);
    }
