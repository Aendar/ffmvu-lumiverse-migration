...
export const EQUIP_STAT_MAP: Record<string, string> = {
  StrBonus: 'Strength',
  AgiBonus: 'Agility',
...
export function numericValue(owner: MutableRecord, key: string, fallback = 0): number {
  const n = Number(tupleValue(owner[key]));
  return Number.isFinite(n) ? n : fallback;
}

export function setNumeric(owner: MutableRecord, key: string, value: number): void {
  const current = owner[key];
  if (Array.isArray(current) && current.length >= 2 && typeof current[1] === 'string') current[0] = value;
  else owner[key] = value;
}

export function applyEquipmentItemStatDelta(owner: MutableRecord, item: MutableRecord, direction: 1 | -1): void {
...
function aggregateEquipmentStatBonuses(equipmentValue: unknown): Record<string, number> {
  const totals: Record<string, number> = {};
  for (const itemKey of Object.keys(EQUIP_STAT_MAP)) totals[itemKey] = 0;
  for (const raw of Object.values(asRecord(equipmentValue))) {
    ...
    for (const itemKey of Object.keys(EQUIP_STAT_MAP)) {
      ...
      if (Number.isFinite(n)) totals[itemKey] += n;
    }
  }
  return totals;
}
 ...
 */
export function applyEquipmentAggregateStatDelta(
  owner: MutableRecord,
  beforeEquipment: unknown,
  afterEquipment: unknown,
): void {
  ...
  for (const [itemKey, statKey] of Object.entries(EQUIP_STAT_MAP)) {
    ...
    if (delta) setNumeric(owner, statKey, numericValue(owner, statKey, 0) + delta);
  }
}
 ...
 */
export function assertEquipmentCapacity(owner: MutableRecord): void {
  ...
  for (const [key, raw] of Object.entries(equipment)) {
    if (!isRecord(raw)) throw new Error('EQUIPMENT_RECONCILIATION_INVALID_ITEM: ' + key);
    const slot = text(raw.Slot || raw.slot).trim();
    if (!slot || SLOT_LIMITS[slot] === undefined) throw new Error('EQUIPMENT_RECONCILIATION_INVALID_SLOT: ' + (slot || '<empty>'));
    if (ACCESSORY_SLOTS.has(slot)) accessoryCount += 1;
    else ordinaryCounts.set(slot, (ordinaryCounts.get(slot) ?? 0) + 1);
  }

  if (accessoryCount > 3) throw new Error('EQUIPMENT_RECONCILIATION_ACCESSORY_LIMIT');
  for (const [slot, count] of ordinaryCounts) {
    if (count > SLOT_LIMITS[slot]) throw new Error('EQUIPMENT_RECONCILIATION_SLOT_LIMIT: ' + slot);
  }
}
 ...
 */
export function recalculateEquipmentDerivedStats(owner: MutableRecord): void {
