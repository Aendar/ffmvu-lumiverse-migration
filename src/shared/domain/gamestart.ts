import type { FFMVUState, LabeledValue } from '../state-schema.js';
...
export function validateGameStartPayload(payload: GameStartPayload): string[] {
  const errors: string[] = [];
  if (!normalizeClock(payload.time)) errors.push('Time must use a valid 24-hour H:MM or HH:MM value');
  if (!Number.isInteger(payload.charisma) || payload.charisma < 80 || payload.charisma > 100) errors.push('Charisma must be an integer from 80 to 100');
  if (!Number.isInteger(payload.level) || payload.level < 1 || payload.level > 140) errors.push('Level must be an integer from 1 to 140');
  if (!Number.isInteger(payload.exp) || payload.exp < 0) errors.push('EXP must be a non-negative integer');
  if (!Number.isInteger(payload.core) || payload.core < 0) errors.push('Core points must be a non-negative integer');
  const values = [payload.stats.str, payload.stats.agi, payload.stats.con, payload.stats.int, payload.stats.wis];
  if (values.some(value => !Number.isInteger(value) || value < 5)) errors.push('Core attributes must be integers >= 5');
  const spent = values.reduce((sum, value) => sum + (value - 5), 0);
  if (spent > 50) errors.push('Core attribute budget exceeded');
  return errors;
}

function tupleSet<T>(object: Record<string, unknown>, key: string, value: T, label: string): void {
  const current = object[key];
  if (!Array.isArray(current) || current.length < 2) object[key] = [value, label] satisfies LabeledValue<T>;
  else current[0] = value;
}

export function applyGameStartPayload(inputState: FFMVUState, payloadInput: GameStartPayload): FFMVUState {
  ...
  const mc = state.Mainchar as unknown as Record<string, unknown>;
  ...
  if (physiology && typeof physiology === 'object' && !Array.isArray(physiology)) {
    (physiology as Record<string, unknown>).LastPhysAt = { Date: payload.date, Time: payload.time };
  }
  ...
  tupleSet(mc, 'Hp_max', hp, 'Max HP'); tupleSet(mc, 'Hp_curr', hp, 'Current HP');
  tupleSet(mc, 'Sta_max', stamina, 'Max Stamina'); tupleSet(mc, 'Sta_curr', stamina, 'Current Stamina');
  tupleSet(mc, 'Mp_max', mp, 'Max MP'); tupleSet(mc, 'Mp_curr', mp, 'Current MP');
