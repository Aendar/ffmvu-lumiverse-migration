...
export function validateStateV160(state: unknown): string[] {
...
function validatePhysiology(value: unknown, path: string, errors: string[]): void {
  if (!isRecord(value)) {
    errors.push(path + ' is not an object');
    return;
  }
  for (const field of ['Hunger', 'Thirst', 'Bladder', 'Arousal']) {
    ...
    if (!Number.isFinite(number) || number < 0 || number > upper) errors.push(path + '.' + field + ' must be a finite 0–' + upper + ' number');
  }
  const last = asRecord(value.LastPhysAt);
  if (!text(last.Date).trim() || !text(last.Time).trim()) errors.push(path + '.LastPhysAt must contain Date and Time');
...
function validateHphGeometry(value: unknown, path: string, errors: string[]): void {
  if (value === undefined) return;
  if (!isRecord(value)) {
    errors.push(path + ' is not an object');
    return;
  ...
  for (const [ownerId, owner] of Object.entries(value)) {
    ...
    for (const field of ['Physiology', 'ErectionCapacity', 'ErectionLevel']) {
      if (owner[field] !== undefined) errors.push(path + '.' + ownerId + '.' + field + ' is retired; HPH stores geometry only');
    }
  }
}
...
export function validateState(state: unknown): string[] {
  ...
  for (const [id, raw] of Object.entries(asRecord(state.Familiar))) {
    ...
    for (const field of ['Hair_Style', 'Personality', 'Physical_Features', 'ExSkill', 'Bio', 'Biography']) {
      if (raw[field] !== undefined) errors.push('Familiar.' + id + '.' + field + ' is retired; use character sources or CurrentHairstyle');
    }
  }
