...
interface ExtractedPatchSpan extends ExtractedModelPatch {
  start: number;
  end: number;
}

function extractJsonPatchSpans(output: string): ExtractedPatchSpan[] {
  const source = String(output ?? '');
  const openMatches = [...source.matchAll(/<JSONPatch>/gi)];
  if (!openMatches.length) return [];
  const lastOpen = source.toLowerCase().lastIndexOf('<jsonpatch>');
  const lastClose = source.toLowerCase().lastIndexOf('</jsonpatch>');
  if (lastClose < lastOpen) throw new Error('MALFORMED_JSONPATCH_ENVELOPE');

  const result: ExtractedPatchSpan[] = [];
  ...
  if (!result.length) throw new Error('MALFORMED_JSONPATCH_ENVELOPE');
  return result;
}
...
export function resolveContinueJsonPatchEvidence(
  preStoredOutput: string,
  generationEndedContent: string | undefined,
  storedOutput: string,
): ContinueJsonPatchEvidence {
  const pre = String(preStoredOutput ?? '');
  const stored = String(storedOutput ?? '');
  if (!stored.startsWith(pre)) throw new Error('CONTINUE_PREFIX_MISMATCH: stored message no longer extends the frozen pre-Continue text');
  const appendedSegment = stored.slice(pre.length);

  let hostContentMode: ContinueJsonPatchEvidence['hostContentMode'] = 'unavailable';
  if (generationEndedContent !== undefined) {
    const raw = String(generationEndedContent);
    if (raw === stored) hostContentMode = 'full';
    else if (raw === appendedSegment) hostContentMode = 'segment';
    else throw new Error('OUTPUT_PATCH_EVIDENCE_MISMATCH: Continue lifecycle content matches neither full stored message nor exact appended segment');
  }
