...
function mergeDefaults(target: MutableRecord, source: MutableRecord): MutableRecord {
  for (const [key, value] of Object.entries(source)) {
    if (target[key] === undefined || target[key] === null) target[key] = clone(value);
    else if (isRecord(target[key]) && isRecord(value)) mergeDefaults(target[key] as MutableRecord, value);
  }
  return target;
}

function normalizeClothingItem(key: string, value: unknown): MutableRecord {
  ...
  if (item.Slot === 'Extra' && !text(item.Placement).trim()) item.Placement = originalSlot || key;
  ...
  for (const field of CLOTHING_STAT_FIELDS) delete item[field];
  return item;
}

function isClothingTombstone(key: string, value: unknown): boolean {
...
function normalizeOutfit(owner: MutableRecord): void {
  ...
  for (const bucket of ['Worn', 'Wardrobe'] as const) {
...
function migrateProjectionPaths(state: MutableRecord): MutableRecord {
  ...
  for (const bucket of ['Hot', 'Warm']) {
    for (const [id, npc] of Object.entries(asRecord(npcs[bucket]))) {
      if (/^npc_\d{4,}$/.test(id) && !npcs[id]) npcs[id] = npc;
    }
    delete npcs[bucket];
  }
  ...
  for (const [id, member] of Object.entries(asRecord(familiar.Hot))) {
    if (!familiar[id]) familiar[id] = member;
  }
  delete familiar.Hot;
  ...
  for (const [id, note] of Object.entries(asRecord(gmNotes.Candidates))) {
    if (!asRecord(gmNotes.Active)[id]) asRecord(gmNotes.Active)[id] = note;
  }
  ...
  for (const [id, seed] of Object.entries(asRecord(chekhov.Candidates))) {
    if (!asRecord(chekhov.Active)[id]) asRecord(chekhov.Active)[id] = seed;
  }
...
function trimRecord(record: unknown, limit: number): MutableRecord {
  const entries = Object.entries(asRecord(record));
  entries.sort((a, b) => recordTurn(b[1]) - recordTurn(a[1]));
  return Object.fromEntries(entries.slice(0, limit));
}

function compactArchive(id: string, record: unknown, turn: number): MutableRecord {
...
function archiveResolved(activeRaw: unknown, archiveRaw: unknown, turn: number, prefix: string): MutableRecord {
  const active = asRecord(activeRaw);
  const nextArchive = asRecord(archiveRaw);
  for (const [id, value] of Object.entries(active)) {
    if (!isRecord(value) || !RESOLVED_STATUSES.has(lower(value.Status))) continue;
...
function inferRelationshipRefs(key: string, relation: MutableRecord): MutableRecord {
  const existingA = text(relation.A).trim();
  const existingB = text(relation.B).trim();
  if (existingA && existingB) return relation;
  ...
  if (inferredA) relation.A = ['user', 'pc', 'mainchar'].includes(lower(inferredA)) ? 'player' : inferredA;
  if (inferredB) relation.B = ['user', 'pc', 'mainchar'].includes(lower(inferredB)) ? 'player' : inferredB;
  return relation;
}
...
function actorAliasMap(state: FFMVUState): Map<string, string> {
  ...
  for (const alias of ['player', 'user', '{{user}}', 'pc', 'mainchar']) add(alias, 'player');
  ...
  for (const [id, npcRaw] of Object.entries(asRecord(state.Narrative.NPCs))) {
    ...
    add(id, id); add(npc.ID, id); add(npc.DisplayName, id); add(npc.Name, id);
    for (const alias of asArray(npc.Aliases)) add(alias, id);
  }
  for (const [id, memberRaw] of Object.entries(asRecord(state.Familiar))) {
    const member = asRecord(memberRaw);
    add(id, id); add(member.ID, id); add(member.Name, id); add(member.DisplayName, id);
    for (const alias of asArray(member.Aliases)) add(alias, id);
  }
  return aliases;
}

function canonicalActorRef(value: unknown, aliases: Map<string, string>): string {
...
export function normalizeState(input: unknown): FFMVUState {
  ...
  for (const member of Object.values(asRecord(state.Familiar))) {
    if (isRecord(member)) normalizeOutfit(member);
  }
  ...
  for (const [id, npcRaw] of Object.entries(asRecord(narrative.NPCs))) {
    ...
    if (Number.isFinite(numericId)) highestNpcId = Math.max(highestNpcId, numericId);
    npcRaw.ID = id;
    ...
    if (!text(npcRaw.DisplayName).trim()) {
      ...
      if (fallbackName) npcRaw.DisplayName = fallbackName;
    }
    if (Array.isArray(npcRaw.CurrentThought)) npcRaw.CurrentThought = npcRaw.CurrentThought.at(-1) || '';
    if (Array.isArray(npcRaw.Knowledge)) npcRaw.Knowledge = npcRaw.Knowledge.slice(-20);
    else if (isRecord(npcRaw.Knowledge)) npcRaw.Knowledge = trimRecord(npcRaw.Knowledge, 20);
  ...
  narrative.Scene.PresentNPCs = uniqueStrings(
    narrative.Scene.PresentNPCs.map(value => canonicalActorRef(value, actorAliases)),
  ).slice(0, 24);

  for (const [relationId, relationRaw] of Object.entries(asRecord(narrative.Relationships))) {
    if (!isRecord(relationRaw)) continue;
