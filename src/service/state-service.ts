...
import { applyJsonPatch, assertPatchResourceLimits, type JsonPatchOperation } from '../shared/json-patch.js';
...
export class StateService {
  ...
  async getProjectionForNode(scope: StateScope, nodeId: string): Promise<{ nodeId: string; stateHash: string; view: Record<string, unknown>; viewHash: string }> { if (!await this.store.isNodeCommitted(scope, nodeId)) throw new Error('NODE_NOT_COMMITTED'); const materialized = await this.materializer.materialize(scope, nodeId); const view = this.projections.get(LEGACY_PROJECTION_VERSION).build(materialized.state) as Record<string, unknown>; return { nodeId: materialized.nodeId, stateHash: materialized.stateHash, view, viewHash: await canonicalHash(view) }; }
}
