...
import { CURRENT_PROJECTION_VERSION, CURRENT_REDUCER_VERSION, LEGACY_PROJECTION_VERSION, LEGACY_REDUCER_VERSION, STATE_SCHEMA_VERSION, type FFMVUState } from '../shared/state-schema.js';
...
import { EVENT_FORMAT_VERSION, PORTABLE_SNAPSHOT_FORMAT, STATE_MIGRATION_DRAFT_FORMAT, type BaseSnapshot, type BaseSnapshotKind, type ChatStoreRevision, type CommitAnchor, type MaterializedState, type PortableSnapshot, type ProjectionSeed, type StateCommit, type StateCommitKind, type StateMigrationDraft, type StateMigrationPreflight, type StateScope, type TranscriptBaseBoundary } from '../persistence/types.js';
...
export class StateService {
  ...
  async exportStateMigrationTemplate(scope: StateScope, nodeId: string): Promise<StateMigrationDraft> {
    if (!await this.store.isNodeCommitted(scope, nodeId)) throw new Error('STATE_MIGRATION_NODE_NOT_COMMITTED');
    const materialized = await this.materializer.materialize(scope, nodeId);
    const artifact = await this.store.readNode(scope, nodeId);
    return {
      format: STATE_MIGRATION_DRAFT_FORMAT,
      source: {
        chatId: scope.chatId,
        nodeId,
        stateHash: materialized.stateHash,
        schemaVersion: artifact.value.reducerVersion,
      },
      targetState: structuredClone(materialized.state),
    };
  }

  private async prepareStateMigration(scope: StateScope, draft: StateMigrationDraft): Promise<{
    parent: MaterializedState;
    patch: JsonPatchOperation[];
    changedPaths: string[];
  }> {
    if (!draft || draft.format !== STATE_MIGRATION_DRAFT_FORMAT) throw new Error('STATE_MIGRATION_FORMAT_INVALID');
    if (!draft.source || draft.source.chatId !== scope.chatId) throw new Error('STATE_MIGRATION_FOREIGN_DRAFT');
    if (!Object.prototype.hasOwnProperty.call(draft, 'targetState') || !draft.targetState || typeof draft.targetState !== 'object' || Array.isArray(draft.targetState)) {
      throw new Error('STATE_MIGRATION_TARGET_INVALID');
    }
    if (typeof draft.source.nodeId !== 'string' || typeof draft.source.stateHash !== 'string' || typeof draft.source.schemaVersion !== 'string') {
      throw new Error('STATE_MIGRATION_SOURCE_INVALID');
    }
    if (draft.note !== undefined && (typeof draft.note !== 'string' || draft.note.length > 512)) throw new Error('STATE_MIGRATION_NOTE_INVALID');
    if (!await this.store.isNodeCommitted(scope, draft.source.nodeId)) throw new Error('STATE_MIGRATION_SOURCE_NOT_COMMITTED');
    const parent = await this.materializer.materialize(scope, draft.source.nodeId);
    if (parent.stateHash !== draft.source.stateHash) throw new Error('STATE_MIGRATION_STALE_SOURCE');
    const artifact = await this.store.readNode(scope, draft.source.nodeId);
    if (artifact.value.reducerVersion !== draft.source.schemaVersion) throw new Error('STATE_MIGRATION_SCHEMA_MISMATCH');

    const reducer = this.reducers.get(CURRENT_REDUCER_VERSION);
    const target = reducer.normalize(draft.targetState);
    const errors = reducer.validate(target);
    if (errors.length) throw new Error('STATE_MIGRATION_TARGET_INVALID: ' + errors.join('; '));
    const patch = buildStatePatch(parent.state, target);
    assertPatchResourceLimits(patch);
    if (!patch.length) throw new Error('STATE_MIGRATION_NO_CHANGE');
    return { parent, patch, changedPaths: patch.map(operation => operation.path || '/').slice(0, 128) };
  }

  async preflightStateMigration(scope: StateScope, draft: StateMigrationDraft): Promise<StateMigrationPreflight> {
    const prepared = await this.prepareStateMigration(scope, draft);
    return {
      source: structuredClone(draft.source),
      targetSchemaVersion: STATE_SCHEMA_VERSION,
      targetReducerVersion: CURRENT_REDUCER_VERSION,
      changedPaths: prepared.changedPaths,
      patchCount: prepared.patch.length,
      historyPreserved: true,
    };
  }

  async applyStateMigration(scope: StateScope, input: ApplyStateMigrationInput): Promise<MaterializedState> {
    const prepared = await this.prepareStateMigration(scope, input.draft);
    return this.commitPatch(scope, {
      parentNodeId: prepared.parent.nodeId,
      expectedParentStateHash: prepared.parent.stateHash,
      patch: prepared.patch,
      kind: 'migration',
      anchor: structuredClone(input.anchor),
      requestId: input.requestId,
      note: input.draft.note ?? 'state-migration-v1.6',
  ...
  async commitPatch(scope: StateScope, input: CommitPatchInput): Promise<MaterializedState> {
    return this.mutex.run(scope, async () => {
      ...
      if (input.requireCurrentPhysicalTip && (
        physical.head.semanticTipNodeId !== input.parentNodeId
        || (input.expectedParentStateHash !== undefined && physical.head.semanticTipStateHash !== input.expectedParentStateHash)
      )) throw new Error('STATE_MIGRATION_STALE_HEAD');
