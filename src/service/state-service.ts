...
export class StateService {
   ...
   */
  private async stageCheckpointBase(scope: StateScope, input: {
    state: unknown;
    transcriptBoundary?: TranscriptBaseBoundary;
    reducerVersion: string;
    projectionVersion: string;
    ...
    return this.mutex.run(scope, async () => {
      ...
      if (errors.length) throw new Error('Invalid checkpoint state: ' + errors.join('; '));
      ...
      if (input.projectionSeed) {
        ...
        if (seedHash !== seed.promptViewHash) throw new Error('CHECKPOINT_PROJECTION_SEED_HASH_MISMATCH');
        projectionSeed = seed;
        projectionBinding = {
   ...
   */
  async stagePortableSnapshotCheckpoint(
    scope: StateScope,
    ...
    if (snapshot.reducerVersion === LEGACY_REDUCER_VERSION) {
      ...
      if (errors.length) throw new Error('PORTABLE_SNAPSHOT_MIGRATION_INVALID: ' + errors.join('; '));
   ...
   */
  async stageLegacyMigrationCheckpoint(
    scope: StateScope,
    input: StageLegacyMigrationCheckpointInput,
  ): Promise<MaterializedState> {
    if (!await this.store.isNodeCommitted(scope, input.parentNodeId)) throw new Error('CHECKPOINT_MIGRATION_SOURCE_NOT_COMMITTED');
    ...
    const errors = reducer.validate(migrated);
    if (errors.length) throw new Error('CHECKPOINT_MIGRATION_TARGET_INVALID: ' + errors.join('; '));
   ...
   * transition.
   */
  async autoMigrateLegacyState(scope: StateScope, input: AutoMigrateLegacyStateInput): Promise<MaterializedState> {
    if (!await this.store.isNodeCommitted(scope, input.parentNodeId)) throw new Error('AUTO_MIGRATION_SOURCE_NOT_COMMITTED');
    const parent = await this.materializer.materialize(scope, input.parentNodeId);
    ...
    if (errors.length) throw new Error('AUTO_MIGRATION_TARGET_INVALID: ' + errors.join('; '));
