...
async function main() {
    ...
    try {
        resolveContinueJsonPatchEvidence(oldPatch, 'different', suffixModePost);
    }
    catch {
        continueMismatch = true;
    }
    ...
    assert(hphResult.state.Narrative.Scene.HPH?.player.Physiology === undefined, 'HPH owner keeps geometry without a persistent physiology subsystem');
