...
import { EVENT_FORMAT_VERSION } from '../../src/persistence/types.js';
import { StateService } from '../../src/service/state-service.js';
const scope = { userId: 'user-a', chatId: 'chat-a' };
let passed = 0;
...
async function rejects(job, fragment) {
    let hit = false;
    try {
        await job();
    }
...
async function main() {
    ...
    const orphanCommit = {
        ...
        id: 'node_orphan', scope, kind: 'gui', anchor: {},
        parentNodeId: orphanGenesis.nodeId,
        parentStateHash: orphanGenesis.stateHash,
        patch: [{ op: 'replace', path: '/Mainchar/Strength/0', value: 99 }],
        patchHash: 'untrusted', reducerVersion: 'FFMVU-1.5.8', resultStateHash: 'untrusted', projectionBinding: { sourceKind: 'node', sourceNodeId: 'node_orphan', sourceStateHash: 'untrusted', projectionVersion: 'FFMVU-1.5.8', promptProtocolVersion: 'ffmvu-model-state-v1', promptViewHash: 'untrusted' }, transactionId: 'tx_orphan', previousStoreRevisionId: null, previousStoreRevisionHash: null, createdAt: new Date().toISOString(),
    };
    ...
    const originalValue = await storage.getJson(originalPath);
