...
async function finalize(state, scope, parent, patch, requestId) {
...
async function main() {
    ...
    const committed = [];
    for (const nodeId of golden.committedNodeIds)
        committed.push(await goldenStore.readCommit(goldenScope, nodeId));
    ...
    for (const nodeId of unrelated.committedNodeIds)
        unrelatedCommits.push(await unrelatedStore.readCommit(oldScope, nodeId));
    ...
    for (const nodeId of stamina.committedNodeIds)
        staminaCommits.push(await staminaStore.readCommit(staminaScope, nodeId));
