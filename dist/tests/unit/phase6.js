...
async function main() {
    ...
    assert(automaticNode.type === 'commit' && automaticNode.value.kind === 'migration'
        ...
        && automatic.state.Mainchar.Physiology.LastPhysAt.Time === '08:00', 'legacy schema upgrade is deterministic and requires no user-supplied migration draft');
    ...
    assert(stagedRestoreNode.type === 'base' &&
        ...
        !('Chekhov' in stagedRestore.state.Narrative), 'legacy portable restore verifies original snapshot then stages migrated v1.7 state with original hashes in provenance');
    const v160Scope = { userId: 'u', chatId: 'v160-physiology-cleanup' };
    const v160State = createDefaultState();
    delete v160State.Mainchar.Physiology;
    ...
    const migratedFamiliar = v170.state.Familiar.evelyn;
    const migratedHph = v170.state.Narrative.Scene.HPH.player;
    assert(v170.state.Mainchar.Physiology.Hunger === 0 &&
        ...
        migratedFamiliar.Physiology.Thirst === 0 &&
        ...
        migratedHph.Scrotum.Position === 'between thighs' && migratedHph.Sex.Active === true, 'v1.6 migration moves physiology to persistent actors, removes redundant familiar profile fields without guessing a hairstyle, and preserves only HPH geometry');
