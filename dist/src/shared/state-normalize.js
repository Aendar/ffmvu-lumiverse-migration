...
export function normalizeStateV158(input) {
    const raw = isRecord(input) ? clone(input) : createLegacyDefaultState();
    migrateProjectionPaths(raw);
    mergeDefaults(raw, clone(LEGACY_DEFAULT_STATE));
    const state = raw;
...
function compactStateText(value, limit = 320) {
...
function normalizeConditions(value, limit) {
    const result = {};
    for (const [id, raw] of Object.entries(asRecord(value))) {
        if (Object.keys(result).length >= limit || !isRecord(raw))
            break;
        if (currentStatus(raw.Status) === 'resolved')
            continue;
        ...
        if (cause)
            entry.Cause = cause;
        if (clearWhen)
            entry.ClearWhen = clearWhen;
        if (lastTouchedTurn !== undefined)
            entry.LastTouchedTurn = lastTouchedTurn;
        result[id] = entry;
    }
    return result;
}
function normalizeInnerThreads(value, limit) {
    const result = {};
    for (const [id, raw] of Object.entries(asRecord(value))) {
        if (Object.keys(result).length >= limit || !isRecord(raw))
            break;
        ...
        if (!subject && !tension)
            continue;
        const entry = { Subject: subject, Stance: stance, Tension: tension, Status: 'active' };
        const lastTouchedTurn = optionalTurn(raw.LastTouchedTurn);
        if (lastTouchedTurn !== undefined)
            entry.LastTouchedTurn = lastTouchedTurn;
        result[id] = entry;
...
function normalizeAgenda(value) {
    if (!isRecord(value) || currentStatus(value.Status) === 'resolved')
        return undefined;
    const agenda = { Status: 'active' };
...
export function normalizeStateV160(input) {
    const legacy = normalizeStateV158(input);
    const raw = clone(legacy);
    mergeDefaults(raw, clone(DEFAULT_STATE_V160));
    ...
    for (const npcRaw of Object.values(npcs)) {
        ...
        if (agenda)
            npcRaw.Agenda = agenda;
        else
            delete npcRaw.Agenda;
    }
    narrative.NPCs = npcs;
    ...
    for (const memberRaw of Object.values(familiar)) {
        ...
        if (agenda)
            memberRaw.Agenda = agenda;
        else
            delete memberRaw.Agenda;
    }
...
function normalizePhysiology(value, date, time, legacyValue) {
    const source = asRecord(value);
    const legacy = asRecord(legacyValue);
    const read = (field) => source[field] !== undefined ? source[field] : legacy[field];
    const bounded = (field) => clamp(read(field), 0, 10);
...
function stripRetiredErectionTracking(value) {
    if (Array.isArray(value))
        return value.map(stripRetiredErectionTracking);
    if (!isRecord(value))
        return value;
...
function legacyHphPhysiology(value, actorId, player = false) {
    ...
    for (const alias of aliases) {
        ...
        if (Object.keys(physiology).length)
            return physiology;
    }
    return {};
}
function normalizeHph(value) {
    const normalized = {};
    for (const [ownerId, ownerRaw] of Object.entries(asRecord(value))) {
        ...
        for (const field of ['Penis', 'Scrotum', 'Sex']) {
            if (owner[field] !== undefined)
                next[field] = stripRetiredErectionTracking(owner[field]);
        }
        if (Object.keys(next).length)
            normalized[ownerId] = next;
    }
...
export function normalizeState(input) {
    ...
    const scene = asRecord(narrative.Scene);
    const mainchar = asRecord(raw.Mainchar);
    mainchar.Physiology = normalizePhysiology(mainchar.Physiology, date, time, legacyHphPhysiology(scene.HPH, 'player', true));
    const familiar = asRecord(raw.Familiar);
    for (const [familiarId, memberRaw] of Object.entries(familiar)) {
        if (!isRecord(memberRaw))
            continue;
        for (const field of LEGACY_FAMILIAR_BIOGRAPHY_FIELDS)
            delete memberRaw[field];
        if (memberRaw.CurrentHairstyle !== undefined) {
            const current = text(tupleValue(memberRaw.CurrentHairstyle)).trim();
            if (current)
                memberRaw.CurrentHairstyle = [current, 'Current Hairstyle'];
            else
                delete memberRaw.CurrentHairstyle;
        }
        memberRaw.Physiology = normalizePhysiology(memberRaw.Physiology, date, time, legacyHphPhysiology(scene.HPH, familiarId));
    }
    raw.Familiar = familiar;
