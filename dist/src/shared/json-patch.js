...
export function assertPatchResourceLimits(operations, limits = DEFAULT_PATCH_RESOURCE_LIMITS) {
    if (operations.length > limits.maxPatchOperations)
        throw new Error('MAX_PATCH_OPERATIONS exceeded');
    if (byteLength(operations) > limits.maxPatchBytes)
        throw new Error('MAX_PATCH_BYTES exceeded');
    for (const operation of operations) {
        if (operation.path.length > limits.maxPointerLength)
            throw new Error('MAX_POINTER_LENGTH exceeded');
        if (operation.path.split('/').length - 1 > limits.maxPointerDepth)
            throw new Error('MAX_POINTER_DEPTH exceeded');
        if ('value' in operation && byteLength(operation.value) > limits.maxSingleValueBytes)
            throw new Error('MAX_SINGLE_VALUE_BYTES exceeded');
    }
}
export function assertModelOperationPolicy(operations) {
    for (const operation of operations) {
        if (!['add', 'replace', 'remove'].includes(operation.op))
            throw new Error('Model operation not allowed: ' + operation.op);
    }
}
export function canonicalizeTupleOperation(state, operation) {
...
export function repairLabeledTuples(current, baseline) {
    ...
    for (const [key, value] of Object.entries(baseline))
        output[key] = repairLabeledTuples(output[key], value);
    return output;
}
export function applyJsonPatch(input, operations) {
    ...
    for (const rawOperation of operations) {
        ...
        if (operation.op === 'add')
            state = pointerAdd(state, operation.path, operation.value);
        else if (operation.op === 'remove')
            state = pointerRemove(state, operation.path);
        else if (operation.op === 'replace')
            state = pointerReplace(state, operation.path, operation.value);
        else if (operation.op === 'copy')
            state = pointerAdd(state, operation.path, pointerGet(state, operation.from));
            ...
            if (JSON.stringify(pointerGet(state, operation.path)) !== JSON.stringify(operation.value))
                throw new Error('JSONPatch test failed: ' + operation.path);
        }
    }
    return state;
}
