...
function addSignal(out, path, value, priority, label, presence = false) {
...
function addCollectionSignals(out, domain, actorId, raw) {
    for (const [id, value] of Object.entries(asRecord(raw))) {
        ...
        if (domain === 'Inventory' || domain === 'Equipment') {
            if (record.Qty !== undefined)
                addSignal(out, `${base}.Qty`, record.Qty, 10, label);
        }
    }
}
function actorSignals(state) {
    const out = new Map();
    const actors = {
        player: state.Mainchar,
    };
    ...
    for (const [id, raw] of Object.entries(asRecord(state.Narrative.Relationships))) {
        ...
        for (const field of ['Bond', 'Sparks', 'Grudge', 'Dynamic']) {
            if (relation[field] !== undefined)
                addSignal(out, `Relationships.${id}.${field}`, relation[field], field === 'Dynamic' ? 4 : 6, id);
        }
    }
    return out;
}
...
export function computeRecentStateHistory(states, options = {}) {
    if (states.length < 2)
        return null;
    ...
    for (let index = 1; index < states.length; index++) {
        ...
        for (const path of paths) {
            ...
            if (!a || !b || sameScalar(a.value, b.value))
                continue;
...
//# sourceMappingURL=state-history.js.map