...
spindle.onFrontendMessage(async (payload, userId) => {
    ...
    if (payload?.type === 'ffmvu_set_enabled') {
        ...
        ensureRegistrations();
        publish(userId, { phase: enabled ? 'armed' : 'disabled', enabled, noPatchProbeArmed: noPatchProbeUsers.has(userId), continueProbeArmed: continueProbeUsers.has(userId), note: enabled ? 'v0.11.1 bridge armed. Composer-mounted StatusMenu, New Game and legacy import use the same branch-aware StateService journal as model generations.' : 'Bridge will not touch generations.' });
        return;
    }
...
spindle.log.info(`[FFMVU] Lumiverse migration bridge v${BRIDGE_VERSION} loaded (v0.11.1 composer StatusMenu + legacy import + branch-safe GUI intents + narrative history context).`);
