...
async function prepareGeneration(context, targetMessageId) {
    ...
    if (isContinue) {
        for (let i = rawAll.length - 1; i >= 0; i--) {
            ...
            if (anchor.storedMessageTextHash !== storedTextHash || index.swipeFingerprints[variantId]?.storedMessageTextHash !== storedTextHash)
                continue;
            continueSnapshot = {
...
async function finalizeContinueModelCommit(payload, pending) {
    ...
    try {
        ...
        const writeContinueEvidence = async (status, modelCommitId, tipNodeId, hashes, resolvesAttemptId) => {
...
const interceptorHandler = async (messages, context) => {
    ...
    const historyMessages = injectNarrativeHistoryContext(injected.messages, pending.assistantNarrativeTimestamps ?? {}, pending.recentChanges ?? null);
...
spindle.onFrontendMessage(async (payload, userId) => {
    ...
    if (payload?.type === 'ffmvu_set_enabled') {
        ...
        ensureRegistrations();
        publish(userId, { phase: enabled ? 'armed' : 'disabled', enabled, noPatchProbeArmed: noPatchProbeUsers.has(userId), continueProbeArmed: continueProbeUsers.has(userId), note: enabled ? 'v0.13.1 bridge armed. Composer-mounted StatusMenu, New Game and legacy import use the same branch-aware StateService journal as model generations.' : 'Bridge will not touch generations.' });
        return;
    }
...
spindle.log.info(`[FFMVU] Lumiverse migration bridge v${BRIDGE_VERSION} loaded (v0.13.1 canonical StatusMenu + safe Variables editor + legacy import + branch-safe GUI intents + narrative history context).`);
